...
PAIRED_ARTIFACT_NAMES = ({"windows_installer", "windows_blockmap"},)
...
def _require_paired_roles(present: Iterable[str], context: str) -> None:
    ...
    have = set(present)
    for pair in PAIRED_ARTIFACT_NAMES:
        carried = pair & have
        if carried and carried != pair:
            missing = sorted(pair - carried)
...
def _artifact_paths(bundle_dir: Path) -> dict[str, Path]:
    ...
    for logical_name, pattern in ARTIFACT_NAMES.items():
        ...
        if len(matches) > 1 or (
            not matches and logical_name in REQUIRED_ARTIFACT_NAMES
        ):
...
def validate_manifest(
    ...
    *,
    ...
    if (
        ...
        or not set(REQUIRED_ARTIFACT_NAMES) <= set(artifacts) <= set(ARTIFACT_NAMES)
    ):
...
def extract_verified_archive(archive_path: Path, output_dir: Path, *, expected_digest: str) -> None:
    ...
    with archive:
        ...
        if (
            not MIN_FILE_COUNT <= len(infos) <= MAX_FILE_COUNT
