...
class TestConfiguredTargets:
    def test_targets_reflect_allowed_open_ids(self) -> None:
        transport = FeishuTransport(
            FakeClient(), allowed_open_ids=["ou_abc", "ou_def"]
        )
...
class TestGroupGate:
    ...
    async def test_group_denied_when_allow_group_false(self) -> None:
        ...
        with patch("kiro_crew.feishu.transport.sel") as mock_sel:
            await t.receive(
                _inbound("ou_abc", "hi", chat_type="group", chat_id="grp1")
            )
        assert dispatched == []
        mock_sel().log_api_access.assert_called_once()
    ...
    async def test_group_denied_when_chat_id_not_allowlisted(self) -> None:
        ...
        with patch("kiro_crew.feishu.transport.sel") as mock_sel:
            await t.receive(
                _inbound("ou_abc", "hi", chat_type="group", chat_id="grp_other")
            )
        assert dispatched == []
        mock_sel().log_api_access.assert_called_once()
    ...
    async def test_group_allowed_when_flag_and_chat_id_match(self) -> None:
        ...
        await t.receive(
            _inbound("ou_abc", "hello", chat_type="group", chat_id="grp_allowed")
        )
...
class TestReceive:
    ...
    async def test_authorized_dispatches_inbound(self) -> None:
        ...
        t = FeishuTransport(
            FakeClient(), allowed_open_ids=["ou_abc"], dispatch=dispatch
        )
