...
class FakeSessions:
    def __init__(
        ...
        *,
        ...
        self._has_session = (
            provider is not None if has_session is None else has_session
        )
        self.acquired: list = []
        self.released: list = []
        self.successes: list = []
...
class FakeHooks:
    ...
    def on_tool_call(
        self, title, *, session_key, agent, tool_kind, raw_params=None,
        command=None, is_shell=False,
    ):
        return _GateResult("")
...
class FakeCtx:
    ...
    def build_message(
        self, text, is_new, key, *, channel_id, agent, resumed,
        runtime_source,
    ):
...
class FakeClient:
    ...
    async def send_reply(self, message_id: str, content: str) -> None:
...
def _cfg(default_agent: str = "", approval_mode: str = "interactive", **kw):
    ...
    return SimpleNamespace(
        agent=SimpleNamespace(
            default_agent=default_agent, approval_mode=approval_mode
        ),
        feishu=SimpleNamespace(
            hard_threshold_pct=95.0, soft_threshold_pct=80.0
        ),
...
class TestTurn:
    ...
    async def test_agent_resolves_to_kirocrew_when_unset(self) -> None:
        ...
        sessions = FakeSessions(provider)
        d = _dispatcher(
            sessions, FakeCtx(), FakeClient(), cfg=_cfg(default_agent="")
        )
    ...
    async def test_cold_start_failure_finalizes_renderer(self) -> None:
        ...
        sessions = FakeSessions(
            provider, raise_on_get=RuntimeError("boom")
        )
        client = FakeClient()
    ...
    async def test_soft_threshold_notice_post_turn(self) -> None:
        ...
        assert any(
            "对话上下文已较长" in content for _, content in client.replies
        )
        # The real answer is also persisted.
        assistant_texts = [
            t for (_, role, t) in conv.appended if role == "assistant"
        ]
    ...
    async def test_hard_threshold_auto_compacts(self) -> None:
        ...
        assert any(
            "自动压缩" in content for _, content in client.replies
        )
