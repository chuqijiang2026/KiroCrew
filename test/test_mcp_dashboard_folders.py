...
class TestTheVerifiedCallerKeyReachesTheRequest:
    ...
    def test_send_carries_the_verified_key(self):
        with patch(
            "kiro_crew.mcp_dashboard._resolve_session_key_strict", return_value=self.VERIFIED
        ), patch("kiro_crew.mcp_dashboard._post", return_value={"delivered": "steered"}) as post:
            _call_tool_inner(
                "session_message_send", {"target": "peer", "message": "hi"}
            )
