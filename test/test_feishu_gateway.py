    ...
    def test_none_falls_back_to_cfg(self) -> None:
        orch = _orch(
            ...
            _cfg=SimpleNamespace(
                agent=SimpleNamespace(approval_mode="interactive")
            ),
        )
        assert _resolve_approval_mode(orch) == APPROVAL_INTERACTIVE
...
class TestMaybeStartFeishu:
    ...
    async def test_noop_when_disabled(
        self, mock_dispatcher, mock_transport, mock_client
    ) -> None:
    ...
    async def test_happy_path(
        ...
        mock_client_cls.assert_called_once_with(
            app_id="app-id-1", app_secret="secret-1"
        )
        fake_client.set_message_handler.assert_called_once_with(
            fake_transport.receive
        )
    ...
    async def test_empty_allow_list_warns_but_starts(
        ...
        assert result is fake_client
        assert any(
            "allowed_open_ids is empty" in r.message for r in caplog.records
        )
