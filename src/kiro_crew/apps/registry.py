...
async def _move_checkout_aside(dest: Path, log_lines: list[str]) -> Path | None:
    ...
    Cancellation safety: a cancellation delivered while awaiting the thread
    can only land before the rename+mtime-refresh started or after both
    completed -- never in between, since they now run as one thread call. If
    the rename had already completed when the cancellation arrived, this
    synchronously moves *aside* back to *dest* so the caller's state is
    unchanged by the attempt; if that undo itself fails, the aside path is
    logged (so it is never silently strandable) before the cancellation is
    re-raised.
    """
    aside = dest.with_name(f"{dest.name}.stale-{uuid.uuid4().hex[:8]}")
    try:
        await asyncio.to_thread(_rename_and_refresh_mtime, dest, aside)
    except OSError as exc:
        log_lines.append(f"Could not move aside the checkout at {dest}: {exc}")
        return None
    except asyncio.CancelledError:
        # Synchronous, not another awaited thread call: the task is already
        # being cancelled, and a fast rename back does not warrant re-entering
        # the async cancellation machinery for a second time.
        if aside.exists():
            try:
                aside.rename(dest)
