 ...
 *
 * A slot-lock refusal, not the readiness 503 that used to fire here: that gate is
 * gone from this endpoint, so a frame showing its message would be evidence for a
 * state the fix makes unreachable.
 */
const REFUSAL = 'sub-agents are running'
      ...
      <MemoryRouter>
        <div data-capture-root className="bg-bg text-text flex flex-col gap-2 py-5 w-[900px]">
          ...
          {scene === 'after' && (
            ...
            >
              <ErrorNotice title="Couldn't continue" message={REFUSAL} onDismiss={() => {}} />
            </div>
          )}
