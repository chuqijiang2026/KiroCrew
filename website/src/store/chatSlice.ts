 ...
 *  entries in. Both spellings are removed from both groups: `safeKey` is
 *  identity for ordinary slot names, and passing an already-rewritten key back
 *  through it is a no-op, so one pass covers a caller holding either form. */
const evictSlotState = (state: ChatState, slotKey: string): void => {
  const spellings = [slotKey, safeKey(slotKey)]
  for (const m of [...slotKeyedMaps(state), ...safeSlotKeyedMaps(state)]) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(sseSlots, (state, action) => {
        ...
        // A live slot is protected under either spelling, since the safe-keyed
        // maps store it rewritten.
        for (const key of [...live]) live.add(safeKey(key))
        // Every map is scanned, not just the bare-keyed ones: a departed slot
        // whose only residue sits in a safe-keyed map would otherwise never be
        // enumerated here and would survive this path. Slot history is scanned
        // alongside them because a slot can survive there after every map entry
        // has already been pruned.
        const cached = new Set([
          ...slotKeyedMaps(state).flatMap(m => Object.keys(m)),
          ...safeSlotKeyedMaps(state).flatMap(m => Object.keys(m)),
          ...(state.slotHistory ?? []),
        ])
