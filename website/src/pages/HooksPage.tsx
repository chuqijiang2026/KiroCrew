...
export default function HooksPage({ embedded }: { embedded?: boolean } = {}) {
  ...
  const [attachHooksScroller, hooksTableEdges] = useScrollEdges<HTMLDivElement>()
          ...
          {hooks.length === 0 ? (
            ...
            <div ref={attachHooksScroller} className="overflow-x-auto">
              {/* This table is AUTO layout (`w-full border-collapse`, no
                  ...
                  a `border-l` on the cell plus a `right-full` gradient child
                  hanging just left of it — both gated on the measured overflow
                  flag, so a table that fits renders neither. Same treatment as
                  the Schedule jobs table (issue #4102). */}
              <table className="w-full border-collapse table-striped">
                <thead>
                  <tr>
                    ...
                    <th className={`sticky right-0 bg-card text-left text-muted text-[12px] uppercase tracking-[.04em] px-2.5 py-2 border-b border-border font-medium w-[160px] ${hooksTableEdges.right ? 'border-l' : ''}`}>
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    ...
                    <tr key={h.id} className={`group/hookrow hover:bg-bg-hover transition-colors ${h.enabled ? '' : 'opacity-50'}`}>
                      ...
                      <td aria-label={i18nT('pages.hooksPage.actions')} className={`sticky right-0 bg-card px-2.5 py-2 border-b border-border text-sm ${hooksTableEdges.right ? 'border-l' : ''}`}>
