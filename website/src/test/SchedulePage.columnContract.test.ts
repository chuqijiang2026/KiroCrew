 ...
 *
 * The second suite pins the sticky Actions treatment (#4089/#4102): the table's
...
describe('SchedulePage jobs table sticky Actions column', () => {
  it('pins the Actions header cell on an opaque background, keeping its width', async () => {
    ...
    expect(cls, 'the border seam must stay overflow-gated: a permanent seam lies on a full-width table')
      .toContain("jobsTableEdges.right ? 'border-l border-border' : ''")
